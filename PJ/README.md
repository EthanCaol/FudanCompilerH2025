...
本编译器项目是RCOMP130014h.01课程的核心实践环节，旨在通过实现完整的编译器工具链，帮助学生深入理解现代编译技术的关键原理。项目采用工业界广泛使用的多阶段处理架构，从词法分析到目标代码生成，涵盖了编译器的全部核心模块。

本项目的独特价值在于：
1. 教学与实践的深度结合：将编译原理的理论知识转化为实际可运行的代码
2. 完整的工具链体验：覆盖从源代码到可执行文件的全过程
3. 现代编译技术实践：包含SSA、寄存器分配等前沿优化技术

---

## 编译器架构

本编译器采用经典的多阶段处理架构，各阶段通过精心设计的数据结构进行信息传递，保持高内聚低耦合的设计原则。完整处理流程如下：

```
词法分析 → 语法分析 → 语义分析 → IR生成 → 指令选择 → SSA转换 → 寄存器分配 → 目标代码生成
```

关键数据结构：
1. **AST（抽象语法树）**：保存源代码的语法结构信息
2. **IR+（扩展中间表示）**：兼具高级语言结构和低级指令特征的中间表示
3. **Quad指令**：接近机器指令的中间表示形式
---

## 技术特点

### 1. 模块化设计
- **清晰的接口定义**：各阶段通过标准化的数据结构接口通信
- **独立的测试能力**：每个模块都可以单独编译和测试
- **灵活的扩展机制**：采用访问者模式实现语义分析，便于添加新的分析功能

### 2. 优化策略
- **SSA形式**：实现基于静态单赋值的中间表示
- **多级优化**：包含常量传播、死代码消除、循环优化等
- **智能寄存器分配**：采用基于图着色的全局寄存器分配算法
---
...
## 示例代码：hw5test5.fmj -> hw5test5.2.ast
```
public int main() {
   class D d;
   d.m();
...
## 1. 类型系统设计
类型系统基于AST_Semant结构体实现，包含三种基本类型：
- INT：整型，用于数值运算和逻辑判断
- ARRAY：整型数组，需记录维度信息
- CLASS：类类型，需记录类名信息
...
## 2. 核心检查机制
类型检查通过AST_Semant_Visitor访问者模式实现，主要功能包括：
1. **符号表查询**：
   - 通过name_maps查询变量/方法的声明信息
...
本编译器项目完整实现了从FDMJ高级语言到ARM汇编的完整编译流程，主要技术成果包括：

1. **前端处理**：
   - 实现基于Flex/Bison的词法语法分析器
   - 构建带语义标注的AST结构
   - 完成包含继承检查的多层次类型系统
...
| 文件后缀                 | 生成阶段       | 内容描述                                              |
| ------------------------ | -------------- | ----------------------------------------------------- |
| `.2.ast.my`              | 语法分析阶段   | 带语义标注的抽象语法树(AST)，包含类型信息和符号表数据 |
| `.3.irp.my`              | IR中间代码生成 | 初始IR+中间表示，保持高级语言结构特征                 |
| `.3.irp-canon.my`        | IR规范化       | 经过规范化的IR+代码，已进行基本块划分和控制流简化     |
| `.4.quad.my`             | Quad指令选择   | 初始Quad指令序列，尚未进行基本块划分                  |
| `.4-xml.quad.my`         | Quad XML表示   | Quad指令的XML格式中间表示，用于跨阶段数据交换         |
| `.4-block.quad.my`       | 基本块划分     | 划分基本块后的Quad程序，已建立控制流图结构            |
| `.4-ssa.quad.my`         | SSA转换        | 静态单赋值形式的Quad代码，包含phi函数                 |
| `.5-ssa-opt.quad.my`     | SSA优化        | 经过优化的SSA形式Quad代码（如常量传播、死代码消除等） |
| `.6-ssa-prepared.my`     | 寄存器分配准备 | 适配ARM调用约定的Quad程序，已完成特殊指令处理         |
| `.6-ssa-prepared.clr.my` | 寄存器分配结果 | XML格式的寄存器分配方案，包含变量-寄存器/栈槽映射关系 |
