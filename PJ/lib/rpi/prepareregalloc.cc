    ...
    for (QuadBlock* block : *quadFuncDecl->quadblocklist) {
        if (block != nullptr && block->quadlist != nullptr) {
            ...
            if (i == 0)
                ...
                i--; // find the position of last stmt
                    ...
                    if (args != nullptr && !args->empty()) { // if not empty, add move instructions
                        for (QuadTerm* arg : *args) {
                            ...
                            if (arg != nullptr) {
                                ...
                                Type new_type;
                                if (new_arg->kind == QuadTermKind::TEMP)
                                    new_type = new_arg->get_temp()->type;
                                else if (new_arg->kind == QuadTermKind::CONST)
                                    new_type = Type::INT; // for now, we assume all args are int
                                else {
                                    cerr << "Error: arg is not a temp or const" << endl;
                                    exit(EXIT_FAILURE);
                                }
            ...
            while (true) {
                ...
                switch (block->quadlist->at(i)->kind) {
                    ...
                    case QuadKind::MOVE_BINOP: { // binop left must be a temp, multiplication must be all temps
                        ...
                        if (move_binop->use == nullptr)
                            move_binop->use = new set<Temp*>();
                        if (new_t != nullptr)
                            move_binop->use->insert(new_t->temp);
                        if (new_t2 != nullptr)
                    ...
                    case QuadKind::CJUMP: { // both sides must be temps
                        ...
                        if (cjump->use == nullptr)
                            cjump->use = new set<Temp*>();
                        if (new_t != nullptr)
                            cjump->use->insert(new_t->temp);
                        if (new_t2 != nullptr)
                            cjump->use->insert(new_t2->temp);
                        block->quadlist->erase(block->quadlist->begin() + i);
                        if (new_t != nullptr) {
                            block->quadlist->insert(block->quadlist->begin() + i, new_move);
                            i++;
