...
{
    if (src != dst && (!InterferenceGraph::isMachineReg(src) || !InterferenceGraph::isMachineReg(dst))) {
...
InterferenceGraph* buildIg(QuadFuncDecl* funcdecl)
{
    ...
    for (QuadBlock* block : *funcdecl->quadblocklist) {
        ...
        for (QuadStm* stmt : *block->quadlist) {
...
#endif
            Temp* dst_temp = nullptr;
            Temp* src_temp = nullptr;
            if (stmt == nullptr)
            ...
            if (stmt->kind == QuadKind::MOVE) { // if it's a move of two temps, add the move pair
                if (stmt->def != nullptr && stmt->def->size() > 0 && stmt->use != nullptr && stmt->use->size() > 0) {
                    for (Temp* def : *stmt->def) {
                        dst_temp = def;
                        break;
                    } // pick the first one (move should have at most one)
            ...
            if (flowInfo->liveout->find(stmt) == flowInfo->liveout->end())
                continue; // skip if no liveout info! should not happen
            ...
            if (stmt->def != nullptr) { // skip if no def info! should not happen
                for (Temp* def : *stmt->def) {
                    ...
                    for (int n : lo) {
                        if (n == def->num)
                            continue;                       // Ignore self-interference
