...
{
#ifdef DEBUG
    cout << "Checking coloring... colors size = " << colors.size() << ", spills size=" << spilled.size()
         << " IG graph size=" << ig->graph.size() << endl;
#endif
    ...
    for (auto it : all_nodes) {
        ...
        for (auto nc : neighbors_colors) {
            if (spilled.find(nc.first) != spilled.end())
                continue; // if it's spilled, don't check the color
            if (nc.second == colors[it]) {
                cout << "Error: Conflict! Node " << it << " has the same color (color=" << colors[it] << ") as its neighbor " << nc.first
                     << " (color=" << nc.second << ")" << endl;
                valid = false; // Not valid coloring
            }
        }
    }
