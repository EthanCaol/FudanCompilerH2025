...
{
    MainMethod* m = (this->mainMethod) ? static_cast<MainMethod*>(mainMethod->clone()) : nullptr;
    vector<ClassDecl*>* cdl_clone = (this->classDeclList) ? cloneList<ClassDecl>(this->classDeclList) : nullptr;
    return new Program(pos->clone(), m, cdl_clone);
}

MainMethod* MainMethod::clone()
{
    vector<VarDecl*>* vdl_clone = (this->varDeclList) ? cloneList<VarDecl>(this->varDeclList) : nullptr;
    vector<Stm*>* sl_clone = (this->stmList) ? cloneList<Stm>(this->stmList) : nullptr;
...
CallStm* CallStm::clone()
{
    ...
    IdExp* i = (this->method) ? static_cast<IdExp*>(method->clone()) : nullptr;
    vector<Exp*>* p = (this->param) ? cloneList<Exp>(this->param) : nullptr;
    return new CallStm(pos->clone(), e, i, p);
}

Continue* Continue::clone() { return new Continue(pos->clone()); }

Break* Break::clone() { return new Break(pos->clone()); }
...
Starttime* Starttime::clone() { return new Starttime(pos->clone()); }

Stoptime* Stoptime::clone() { return new Stoptime(pos->clone()); }
...
CallExp* CallExp::clone()
{
    ...
    IdExp* n = (this->method) ? static_cast<IdExp*>(method->clone()) : nullptr;
    vector<Exp*>* p = (this->param) ? cloneList<Exp>(this->param) : nullptr;
...
BoolExp* BoolExp::clone() { return new BoolExp(pos->clone(), val); }

This* This::clone() { return new This(pos->clone()); }
...
GetInt* GetInt::clone() { return new GetInt(pos->clone()); }

GetCh* GetCh::clone() { return new GetCh(pos->clone()); }
